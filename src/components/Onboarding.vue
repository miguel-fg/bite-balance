<template>
  <div
    v-if="props.visible"
    class="z-50 inset-0 fixed flex items-center justify-center px-4 bg-ui-gray-100"
  >
    <div
      class="rounded-md md:px-12 md:py-6 md:shadow-md md:w-6/12 md:bg-[#F9F9F9] md:max-w-md"
    >
      ...
      <div v-else-if="currentQuestion === 5" class="flex flex-col gap-12">
        ...
        <div>
          ...
          >
            ...
            <div class="flex justify-between py-4">
              <span class="font-bold text-primary-700">Gender:</span>
              {{ gender }}
            </div>
          </div>
          <p
        ...
        <div class="flex justify-end gap-4">
          <button @click="prevQuestion" class="btn text-ui-gray-700">
            Back
          </button>
          <button
            @click="$emit('closeOnboarding')"
